...
namespace Client
{
    public partial class SinglePlayForm : Form
    {
        ...
        List<Horse> playerList = new List<Horse>();
        ...
        private void EraseEllipse(List<Point> _list, List<Horse> _playerList)
        {
            Graphics g = this.boardPicture.CreateGraphics();
            refresh();
            for (int i = 0; i < _list.Count; i++)
            {
                board[_list[i].X, _list[i].Y] = _playerList[i];
                if (_playerList[i] == Horse.BLACK)
                {
                    SolidBrush brush = new SolidBrush(Color.Black);
                    g.FillEllipse(brush, _list[i].X * rectSize, _list[i].Y * rectSize, rectSize, rectSize);
                }
                else
                {
                    SolidBrush brush = new SolidBrush(Color.White);
